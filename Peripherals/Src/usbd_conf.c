...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle) {
    if (pcdHandle->Instance == USB_OTG_FS) {
        ...
        /** peripherals clock already initialized */
        ...
        */
        ...
        while ((RCC->AHB2ENR & RCC_AHB2ENR_OTGFSEN) != RCC_AHB2ENR_OTGFSEN);

        /* Enable VDDUSB */
        if ((RCC->APB1ENR1 & RCC_APB1ENR1_PWREN) == 0U) {
            RCC->APB1ENR1 |= RCC_APB1ENR1_PWREN;
            while ((RCC->APB1ENR1 & RCC_APB1ENR1_PWREN) != RCC_APB1ENR1_PWREN);

            PWR->CR2 |= PWR_CR2_USV;
            while ((PWR->CR2 & PWR_CR2_USV) != PWR_CR2_USV);
...
{
  if (GPIO_Pin == ((uint16_t) 0x0200)) // GPIO_PIN_9
  {
