...
// enable pushbutton polling input & interrupts
static void BluePushbutton_Init(void) {
...
static void LCD_GPIO_Init(void) {
    ...
    LCD_MOSI_GPIO->MODER &= ~GPIO_MODER_MODE7;
    LCD_MOSI_GPIO->MODER |= GPIO_MODER_MODE7_1;
    LCD_SCK_GPIO->MODER &= ~GPIO_MODER_MODE3;
    LCD_SCK_GPIO->MODER |= GPIO_MODER_MODE3_1;
    LCD_MOSI_GPIO->AFR[0] &= ~GPIO_AFRL_AFRL7;
    LCD_MOSI_GPIO->AFR[0] |= (GPIO_AFRL_AFSEL7_2 | GPIO_AFRL_AFSEL7_0);
    LCD_SCK_GPIO->AFR[0] &= ~GPIO_AFRL_AFRL3;
    LCD_SCK_GPIO->AFR[0] |= (GPIO_AFRL_AFSEL3_2 | GPIO_AFRL_AFSEL3_0);
    ...
    // set no PUPD
    GPIOA->PUPDR &= ~(GPIO_PUPDR_PUPD7 | GPIO_PUPDR_PUPD9);
    GPIOB->PUPDR &= ~(GPIO_PUPDR_PUPD3 | GPIO_PUPDR_PUPD6);
    ...
    LCD_SCK_GPIO->OTYPER &= ~GPIO_OTYPER_OT3;
    LCD_RST_GPIO->OTYPER &= ~GPIO_OTYPER_OT9;

    // set very high output speed
    GPIOA->OSPEEDR |= (GPIO_OSPEEDR_OSPEED7 | GPIO_OSPEEDR_OSPEED9);
    GPIOB->OSPEEDR |= (GPIO_OSPEEDR_OSPEED3 | GPIO_OSPEEDR_OSPEED6);
